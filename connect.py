...
class spider():
    ...
    def FileComparison(self):
        ...
        directoryconfs = getconfinfo.process_ovpn_directory(directory)
        temp = os.path.join(os.getcwd(), "temp")

        tempconfs = getconfinfo.process_ovpn_directory(temp)

        for temp_ip_port in tempconfs:
            for direct_ip_port in directoryconfs:
                if temp_ip_port == direct_ip_port:
                    os.remove(directoryconfs[direct_ip_port])
                    print("dele" + directoryconfs[direct_ip_port])

        for file in os.listdir(temp):
            path_file = os.path.join(os.getcwd(), "temp", file)
            new_path_file = os.path.join(os.getcwd(), "directory", file)
            if os.path.exists(new_path_file):
                with open(path_file, "r") as text:
                    content = text.read()
                    ip_port_match = re.search(r"remote\s+([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)\s+(\d+)", content)
                    if ip_port_match:
                        ip = ip_port_match.group(1)
                        port = ip_port_match.group(2)
                        newname = ip + "---" + file
                        new_path_file = os.path.join(os.getcwd(), "directory", newname)
